...
`Dockerfile.emulator` copia questa cartella nell'immagine e avvia
l'emulatore con `--import=/app/seed-data`: il container parte già
popolato, senza dover ricordarsi di lanciare `npm run seed:emulator` a
parte — così il menù locale (via `npm run dev:local` o la suite E2E) non è
mai vuoto quando si usa il container senza credenziali reali.
